...
Parses Nginx Combined Log Format and error logs.
"""

import re
from datetime import datetime, timezone

# Nginx Combined Log Format: IP - User [Date] "Method URL Protocol" Status Bytes "Referer" "User-Agent"
NGINX_COMBINED_REGEX = re.compile(
    r'^(\S+)\s+\S+\s+(\S+)\s+\[([^\]]+)\]\s+"(\S+)\s+(\S+)\s+([^"]+)"\s+(\d{3})\s+(\d+|-)\s+"([^"]*)"\s+"([^"]*)"$'
)

def parse_nginx_log(raw_log: str) -> dict:
    """Parse Nginx combined log line."""
    match = NGINX_COMBINED_REGEX.match(raw_log.strip())
    if match:
